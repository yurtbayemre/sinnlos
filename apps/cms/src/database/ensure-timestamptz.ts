 ...
 *  2. db.init, beforeSync, db.schema.sync() :347-364: user migrations (the
 *     one-time legacy repair converts every legacy column itself), internal
 *     migrations, then the schema diff creates new columns naive.
 *  3. afterSync hook :391 -> convertNaiveColumns(): one short transaction
 ...
 *     holds can time out; that is logged and retried in step 4.
 *  4. user bootstrap() :408 (after the content-types store is saved and the
...
/** Wires the guard into Strapi's afterSync hook (call from register()). */
