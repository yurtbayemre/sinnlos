 ...
 * returns every other key of the client query verbatim (@strapi/utils 5.49
 * sanitize/index.js sanitizeQuery: cloneDeep + pick only under
 * strictParams). The core content types are safe anyway, because the
 * document service drops unknown root params, but users-permissions and
 * upload skip the document service: their services run the sanitized query
 * through `query-params.transform`, which keeps unknown keys
 * (`{ ...rest, ...query }`, convert-query-params.js), and hand the result
 * to `strapi.db.query(uid).findMany/findOne`. The DB layer reads the raw
 * query-builder keys `where`, `select`, `orderBy`, `groupBy` and `offset`
 * (@strapi/database query-builder.js init). So
 *   `GET /api/users?where[department][pages][body][$startsWith]=Secret`
 *   `GET /api/users?where[resetPasswordToken][$startsWith]=a1`
 * reached the database with no private-field, restricted-relation or FX05
 * check at all: a blind row-count oracle over hidden wiki bodies, sensitive
 * user fields and even password hashes and reset tokens, for every role
 * that holds `user.find` (all of them). The FX05 guard only inspected
 * `filters`, `sort` and `populate`, so it missed this.
 *
 ...
 * controller calls `sanitize.query(query, schema, { auth })` without
 * forwarding strictParams, so the pick never runs on /api/users.
 */
